...
// GetCurrentUserDomainEmail is a best-effort guess unless you configure it for your own
// organization's domains - shown here with a made-up mapping.
...
// Web-context methods need an IWebRequestContext. There's no real HttpContext in a console
// app, so this fakes one - see README.md for a real Microsoft.AspNetCore.Http adapter
// (and a System.Web one for classic ASP.NET) to use in an actual web project.
...
internal sealed record FakeWebRequestContext(
    string? UserIdentityName,
    string? ForwardedForHeader,
    string? RemoteAddress,
    string? Scheme,
    string? Authority,
    string? ApplicationPath) : IWebRequestContext;
