...
public sealed class DataverseClient
{
    ...
    // Not [GeneratedRegex] because this library multi-targets net6.0, which predates
    // the regex source generator (added in .NET 7).
    private static readonly Regex EntityIdRegex = new(@"\(([0-9a-fA-F\-]{36})\)\s*$", RegexOptions.Compiled);
    ...
    public Task<DataverseApiResult> ListAsync(string entitySetName, string? odataQueryString = null, CancellationToken cancellationToken = default) =>
    ...
    public Task<DataverseApiResult> GetAsync(string entitySetName, Guid id, string? odataQueryString = null, CancellationToken cancellationToken = default) =>
    ...
    public Task<DataverseApiResult> CreateAsync(string entitySetName, string jsonBody, CancellationToken cancellationToken = default) =>
    ...
    public Task<DataverseApiResult> UpdateAsync(string entitySetName, Guid id, string jsonBody, CancellationToken cancellationToken = default) =>
        SendAsync(HttpMethod.Patch, BuildUrl(entitySetName, id, null), jsonBody, cancellationToken);

    /// <summary>DELETE a record by id.</summary>
    public Task<DataverseApiResult> DeleteAsync(string entitySetName, Guid id, CancellationToken cancellationToken = default) =>
    ...
    private async Task<DataverseApiResult> SendAsync(HttpMethod method, string url, string? jsonBody, CancellationToken cancellationToken)
    {
        ...
        using (response)
        {
            ...
            return new DataverseApiResult
            {
                ...
                ErrorMessage = response.IsSuccessStatusCode ? null : $"Dataverse returned HTTP {(int)response.StatusCode}."
            };
        }
    }
    ...
    private async Task<string> GetAccessTokenAsync(CancellationToken cancellationToken)
    {
        ...
        var context = new TokenRequestContext([$"{_resourceUrl}/.default"]);
