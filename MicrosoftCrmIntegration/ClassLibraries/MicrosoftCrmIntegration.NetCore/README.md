# MicrosoftCrmIntegration.NetCore
...
to `MicrosoftCrmIntegration.NetCore.csproj`.
...
public sealed class DataverseApiResult
{
    ...
    public int StatusCode { get; init; }         // Dataverse's HTTP status, or 0 if the request never reached Dataverse
    public string Body { get; init; }            // raw JSON Dataverse returned (success or error)
    public Guid? CreatedEntityId { get; init; }  // parsed from the OData-EntityId header on a successful create
...
services.AddSingleton(sp => new DataverseClient(
    resourceUrl, tenantId, clientId, clientSecret,
