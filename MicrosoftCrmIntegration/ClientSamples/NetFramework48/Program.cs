// Sample client for the MicrosoftCrmIntegration Azure Function, written for
...
namespace MicrosoftCrmIntegrationClientSample.NetFramework48
{
    internal static class Program
    {
        ...
        private static async Task<int> Main()
        {
            ...
            try
            {
                await ListAccountsAsync(functionBaseUrl);
                await CreateContactAsync(functionBaseUrl);
                return 0;
            }
            catch (Exception ex)
            {
        ...
        private static async Task ListAccountsAsync(string baseUrl)
        {
            var url = baseUrl.TrimEnd('/') + "/dataverse/accounts?$select=name,accountnumber&$top=5";
            using (var response = await Client.GetAsync(url))
            {
                var responseBody = await response.Content.ReadAsStringAsync();
                Console.WriteLine("[List accounts] HTTP {0} - {1}", (int)response.StatusCode, responseBody);
            }
        }

        /// <summary>Calls POST /dataverse/contacts with a Dataverse-shaped request body.</summary>
        private static async Task CreateContactAsync(string baseUrl)
        {
            const string json = @"{
                ""firstname"": ""Jane"",
                ""lastname"": ""Doe"",
            ...
            using (var response = await Client.PostAsync(baseUrl.TrimEnd('/') + "/dataverse/contacts", content))
            {
                var responseBody = await response.Content.ReadAsStringAsync();
                Console.WriteLine("[Create contact] HTTP {0} - {1}", (int)response.StatusCode, responseBody);
            }
        }
    }
}
