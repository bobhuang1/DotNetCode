...
`/shopify/{resource}[/{id}]`). Two separate samples are provided because the
idiomatic way to create and use an `HttpClient` differs between the two
platforms:
...
Both samples call the same two example operations (list customers, create a
customer) - only the client plumbing differs. See the comment block at the
